...
import CuratedFavoritesClient from "./CuratedFavoritesClient";
...
async function getProducts() {
  const { data } = await adminDb
    .from("products")
    .select("id, title, category_id, price, in_stock, image_url, is_featured, featured_order")
    .order("title", { ascending: true });
  return data || [];
}

export default async function CuratedFavoritesPage() {
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="border-b bg-white sticky top-0 z-10">
        <div className="px-3 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center gap-3 sm:gap-4">
          ...
          <span className="text-slate-600 text-sm hidden sm:inline">Curated Favorites</span>
        </div>
      </header>

      <main className="p-3 sm:p-6 lg:p-8 max-w-7xl mx-auto">
        <CuratedFavoritesClient initialProducts={products} />
      </main>
    </div>
  );
}
