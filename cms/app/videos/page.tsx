...
async function getVideos() {
  const { data } = await adminDb.from("videos").select("*").order("display_order", { ascending: true });
  return data || [];
}
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="border-b bg-white sticky top-0 z-10">
        <div className="px-3 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center gap-3 sm:gap-4">
          ...
          <span className="text-slate-600 text-sm hidden sm:inline">Artisan Stories</span>
        </div>
      </header>
